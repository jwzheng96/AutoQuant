...
case "${1:-}" in
  ...
  migrate)
    ...
    "${compose[@]}" exec -T postgres sh -c \
      'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' \
      < "$project_root/migrations/postgres/038_qmt_canary_order_staging.sql"
    "${compose[@]}" exec -T postgres sh -c \
      'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' \
      < "$project_root/migrations/postgres/039_qmt_canary_remark_recovery.sql"
    "${compose[@]}" exec -T postgres sh -c \
      'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' \
      < "$project_root/migrations/postgres/040_qmt_callback_inbox.sql"
    "${compose[@]}" exec -T postgres sh -c \
      'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' \
      < "$project_root/migrations/postgres/041_qmt_callback_persistence_receipts.sql"
    "${compose[@]}" exec -T postgres sh -c \
      'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' \
    ...
    "${compose[@]}" exec -T postgres sh -c \
      'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' \
      < "$project_root/migrations/postgres/044_low_volatility_forward_evaluation.sql"
    "${compose[@]}" exec -T postgres sh -c \
      'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' \
      < "$project_root/migrations/postgres/045_low_volatility_forward_evaluation_dataset.sql"
    "${compose[@]}" exec -T postgres sh -c \
      'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' \
      < "$project_root/migrations/postgres/046_low_volatility_paper_candidate_approvals.sql"
    "${compose[@]}" exec -T postgres sh -c \
      'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' \
      < "$project_root/migrations/postgres/047_low_volatility_paper_daily_signals.sql"
    "${compose[@]}" exec -T postgres sh -c \
      'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' \
      < "$project_root/migrations/postgres/048_low_volatility_execution_compatibility.sql"
    "${compose[@]}" exec -T postgres sh -c \
      'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' \
      < "$project_root/migrations/postgres/049_low_volatility_execution_compatibility_runs.sql"
    "${compose[@]}" exec -T postgres sh -c \
      'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' \
      < "$project_root/migrations/postgres/050_low_volatility_paper_deployment_contracts.sql"
    "${compose[@]}" exec -T postgres sh -c \
      'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' \
      < "$project_root/migrations/postgres/051_low_volatility_decision_time_signals.sql"
    "${compose[@]}" exec -T postgres sh -c \
      'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' \
      < "$project_root/migrations/postgres/052_qmt_clock_attestations.sql"
