...
异步委托响应的订单关联只能消费已经通过完整哈希链回放的收件箱事件，并同时核对 logical
account、`seq`、`order_id` 和预提交 `order_remark`。收件箱已落库但关联写入前崩溃时，
重启进程可幂等重放；协调器不得用未落库事件、错误 remark 或错误账号直接调用底层关联
账本绕过持久化处理器。

单消费者协调器对内存队列使用 reserve/persist/ack 协议。只有批次中所有回调都持久化后
才能 ack；部分成功、租约丢失、任务取消或数据库异常会 release 同一批次以供精确重试，
溢出流则永远不能 ack。进程在同一 lease generation 中重启时，必须在注册供应商回调前
